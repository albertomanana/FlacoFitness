...
@Service
public class MaquinaPhotoStorageService {
    ...
    private final Path maquinasUploadPath;
    private final DataSize maxImageSize;
    private final String uploadDir;

    public MaquinaPhotoStorageService(@Value("${upload.dir}") String uploadDir,
                                      @Value("${upload.max-image-size:5MB}") DataSize maxImageSize) {
        this.uploadDir = uploadDir;
        this.maxImageSize = maxImageSize;
        this.maquinasUploadPath = Paths.get(uploadDir, "maquinas").toAbsolutePath().normalize();
    }

    public String guardarFotoMaquina(Long maquinaId, MultipartFile foto, String fotoActualPath) {
        validarFoto(foto);

        try {
            Files.createDirectories(maquinasUploadPath);

            String extension = resolverExtension(foto);
            String fileName = "maquina-" + maquinaId + "-" + UUID.randomUUID() + extension;
            Path destino = maquinasUploadPath.resolve(fileName).normalize();

            if (!destino.startsWith(maquinasUploadPath)) {
            ...
            return Paths.get(uploadDir, "maquinas", fileName).toString().replace("\\", "/");
        } catch (IOException ex) {
            throw new BusinessValidationException("No se pudo almacenar la imagen de la maquina");
        }
    }
    ...
    private void eliminarFotoAnterior(String fotoActualPath, Path nuevaFoto) throws IOException {
        ...
        if (fotoAnterior.startsWith(maquinasUploadPath) && !fotoAnterior.equals(nuevaFoto)) {
